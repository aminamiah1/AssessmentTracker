describe("Edit User Form", () => {
    ...
    it("allows a ps-team member to  edit a user", () => {
      ...
      cy.intercept("POST", "/api/ps-team/create-users", {
        statusCode: 200,
        body: { name: "New User Test", email: "newusertest@example.com", roles: "module_leader", password: "example"},
      }).as("addUser");
      cy.get('[data-cy="CreateUser"]').click()

      // Edit the new user
      cy.get('table').should('be.visible').contains('td', 'New User Test').next().next().next().click()
      ...
      cy.intercept("POST", "/api/ps-team/create-users", {
        statusCode: 200,
        body: { name: "New User 2", email: "newuser2@example.com", roles: "module_leader", password: "example"},
      }).as("addUser");
      ...
      cy.get('table').should('be.visible').contains('td', 'New User 2').next().next().click()
    });
});
